...
class Model(object):
    ...
    """
    # Class variable pcolorgivendist
    # The initial probabilities for distance <= 7
    # This dictionary is extended in __init__ to include all values allowed
    # for a game of the given size
    ...
    def pcolorgivendist(self, color, dist):
        """
        Return the conditional probability of the given color given the
        distance
        :param color: (string) sensor reading
        :param dist: distance from the sensor location to a given position
...
class Game(object):
    ...
    def diving_mode(self):
        """
        Set the game to diving mode - the method is invoked when the "DIVER"
        button is clicked.
